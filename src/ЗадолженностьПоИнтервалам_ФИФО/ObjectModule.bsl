...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	ПараметрыРегистрации.Вставить("Наименование", 		НСтр("ru = '<АМ> Задолженность ФИФО по интервалам (версия 4.2)'"));
	ПараметрыРегистрации.Вставить("Версия", 			"4.2");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", 	ИСТИНА);
	ПараметрыРегистрации.Вставить("Информация", 		НСтр("ru = 'Отчет: ""Задолженность ФИФО по интервалам (версия 4.2)""'"));
	
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	
	ДобавитьКоманду(ТаблицаКоманд,
					НСтр("ru = '<АМ> Задолженность ФИФО по интервалам (версия 4.2)'"),
					"<АМ> Задолженность ФИФО по интервалам (версия 4.2)",
					"ОткрытиеФормы",
					Истина,
					"");
...
Процедура РаспаковкаТекстаЗапроса() Экспорт
	...
	РазмерШага = 536870912;
	СуществующийТекст = СхемаКомпоновкиДанных.НаборыДанных.НаборДанных3.Элементы[0].Запрос;
	Если СтрНайти(СуществующийТекст, "Шаг" + Формат(РазмерШага / 2, "ЧГ=0")) > 0 Тогда 
		Возврат;
	КонецЕсли;
